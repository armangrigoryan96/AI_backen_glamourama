...
with image_blocks as demo:
    ...
    with gr.Row():
        with gr.Column():
            imgs = gr.Image(sources='upload', type="pil")
            
        with gr.Column():
            garm_img = gr.Image(sources='upload', type="pil")
            dress=gr.Dropdown(choices=dresses, label="Select a category") # Define the dropdown input

        with gr.Column():
            image_out = gr.Image(elem_id="output-img",show_share_button=False)

    with gr.Column():
        try_button = gr.Button(value="Virtual clothing")
        denoise_steps = 30
        seed = 0



    try_button.click(fn=start_tryon, inputs=[imgs, garm_img, dress], outputs=[image_out], api_name='virtual-dressing')
