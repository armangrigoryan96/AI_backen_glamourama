...
print("_____________________",torch.cuda.device_count())
...
openpose_model = OpenPose(0)
 ...
 #   print(f"_________Using {torch.cuda.device_count()} GPUs!_________")
  #  openpose_model = torch.nn.DataParallel(openpose_model)

#if isinstance(openpose_model, torch.nn.DataParallel):
 #   openpose_model = openpose_model.module
...
#if isinstance(pipe.unet_encoder, torch.nn.DataParallel):
 #   pipe.unet_encoder = pipe.unet_encoder.module
...
def start_tryon(dict,garm_img,dress,category, is_checked,is_checked_crop,denoise_steps,seed):
    ...
    pipe.to(device) #avel
    pipe.unet_encoder.to(device) #avel
...
def virtual_dressing():
    try:
        ...
        res_image  = start_tryon(dict=person,dress="Tshirt", garm_img=cloth, category=position,is_checked=True, is_checked_crop=False, denoise_steps=32, seed=0)
