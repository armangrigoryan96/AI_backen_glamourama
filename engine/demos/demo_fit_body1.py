...
def main(inputpath,savefolder, tPose=True, saveVis=True,saveImages=True):
    ...
    # load test images 
    testdata = TestData(inputpath, body_detector='rcnn')
    ...
    for i, batch in enumerate(tqdm(testdata, dynamic_ncols=True)):
        ...
        if saveObj or saveImages is not None:
        ...
        if saveVis:
            ...
            visdict = visualizer.render_results(opdict, data['body']['image_hd'], overlay=True, moderator_weight=moderator_weight, use_deca=False)
            # show cropped parts 
            if showParts:
            ...
            cv2.imwrite(os.path.join(savefolder, f'{name}_vis.jpg'), visualizer.visualize_grid(visdict, size=1024))
